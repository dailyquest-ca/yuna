...
*Updated: 2026-08-07 (UTC−7, permanent)*
...
**The levered pool (ruled 2026-08-06):** drawn levered capital is never idle — its resting state is the ETF (`config.levered_etf`). In one sentence: **borrowed dollars buy the qualified name when it's trading behind the market, and hand it back to the ETF when it's run well ahead of it.** A single name takes part or all of the pool only when it clears the bar above (ruled PASS · CCN ≥ 85 · at/below hurdle · a facility that permits single names) **and** has lagged the index; it reverts when it runs well ahead, or stops qualifying (CCN < 70 at a filing recompute, or an exit ruling) — the score-break revert applies regardless of relative position. No cycle-frequency cap; no discount bar beyond the hurdle. Yuna proposes and logs every cycle; Zak places both legs; utilization moves only on Zak's word — the callable rules above are untouched. Every non-registered swap carries its tax flag. Lag/lead thresholds and window live in Config (`levered_cycle_params`), set only by Zak's ruling — until set, the brief carries a levered status line (utilization per facility, headroom, holdings) and proposes nothing.
...
**Data confidence:** never assume a missing value — drop the component, renormalize remaining weights to 100, mark the name as scored on 2 of 3. An incompletely-scored name is capped at the bottom of its size band and requires manual sign-off — the sign-off is a logged ruling (ruled 2026-08-06): for a growth-derived name the blind C2 PASS ruling itself is the sign-off; for a name scored on 2 of 3 it is an explicit sign-off ruling by the session that writes or arms its ticket. The pipeline arms a flagged name unblocked only when its sign-off exists in `rulings`. **The compounding engine never routes here** — renormalizing away the engine pays a name for being unmeasurable (missingness travels with the very components that survive); the engine has its own waterfall (§3.1).
...
**Clock convention:** stored and scheduled in **UTC** · planned around market time (**ET**) · quoted to Zak in **PT**. **B.C. is permanently UTC−7** — it stopped changing its clocks on 2026-03-08 (ruled 2026-08-06) — so a UTC cron *is* a PT appointment, permanently: no session hour drifts, and no clock guard is needed. **New York still shifts and B.C. no longer shifts with it**, so what moves twice a year is the market under Zak's clock, not the session: the US open is **6:30 AM PT** from March to early November and **7:30 AM PT** the rest of the year, and a session reads the bell off the current ET offset rather than from memory. Appointments are verified against the **ET** regimes; the PT side is now fixed. **The chain has no clock; the sessions keep appointments** (ruled 2026-08-05). Only the ingests are scheduled — `score` → `check` → `compose` → `notify` chain off them by `needs:` in `pipeline.yml`, so the ordering is a data dependency and cannot invert however late Actions queues the night. **Order and currency are the guarantees; punctuality is not.** Each session fires at a fixed hour chosen to sit after the chain and opens with the freshness line — a session that beats the chain says so rather than speaking stale. On the Saturday it rebuilds, `ingest-universe` runs before the weekly rank, so that week's rankings use the fresh universe.
...
| **Morning chat** | ~4:30 AM Mon–Fri — read 5–6, acted before the bell | Pre-composed brief waiting in this project (minimal scheduled task, one read); typing **morning** in any project chat rebuilds it — the fallback that always works. Yuna overlays live quotes, rules anything newly buyable, ships tickets. Running 2–3 hours ahead of the open, gap reads are **provisional** and say so | R1 |
...
### 5.1 R1 — Morning chat (weekdays ~4:30 AM PT)
...
| 3 | **Gaps ±7%** | Momentum gapped below its stop-limit → order unfilled → **manual exit at open** ticket · Compounder gapped down → check invalidators + hurdle (a gap can create an add) · at 4:30 the pre-market print is thin — every gap read is **provisional**, confirmed at the bell before Zak acts |
...
- [x] First rulings pass — the at-hurdle backlog — *done 2026-08-06: 66 blind C2 rulings + 5 Phase 0 rulings logged; 2 escalations (TSM data defect · RHLD too young to rule)*
- [ ] Execute Phase 0 (§6) — *in flight: exits ticketed (AVGO · ISRG · CNQ) and Step 3 slate shipped (BKNG · TW · HLNE), awaiting Zak's fills; TSM held-unruled pending the FX work order*
...
- **The database's timezone tables are behind the law.** Postgres on Supabase still models the pre-2026 B.C. rule, so `at time zone 'America/Vancouver'` returns UTC−8 for winter dates — `2026-12-15 11:30 UTC` renders as 03:30 when the true local time is 04:30. Harmless today: every stamp is UTC by law (§4.1), and no view or function in the store converts to PT (checked 2026-08-06). But PT rendering must never be delegated to the database until its tzdata catches up.
...
| Date | Entry |
|---|---|
| 2026-08-07 | **The sweep of the chat rulings — sign-off defined, the levered pool becomes law, Phase 0 slate shipped.** §3.3's manual sign-off gets its mechanism (ruled 2026-08-06): for a growth-derived name the blind C2 PASS ruling **is** the sign-off; a 2-of-3 name needs an explicit logged sign-off ruling; the pipeline arms a flagged name only when the sign-off exists in `rulings` — this had gagged all 13 armed compounder entries behind a ritual that didn't exist. §2.5 gains **the levered pool** (ruled 2026-08-06): always deployed, ETF resting state, single names only at the existing CCN ≥ 85 bar and only as a relative-value cycle against the index — in when lagging, back to the ETF when well ahead or disqualified; Yuna proposes, Zak places, utilization his alone; lag/lead thresholds live in `config.levered_cycle_params`, set only by his ruling, status-line-only until then. Mechanics rulings the same day, recorded in runbooks, no plan numbers moved: ruling-driven exits default to **marketable limits** (0.3% inside the last print, recomputed at placement — gap drills stay market-at-open, §4.6); **The Book**, the portfolio-overview artifact, rebuilds daily with the morning brief (R1 step 10, spec `runbooks/the_book_spec.md`). First rulings pass complete (66 C2 + 5 Phase 0, two escalations); Phase 0 Step 3 slate shipped under §6 (BKNG · TW · HLNE at 12%). Code work orders for the pipeline in `status/2026-08-07_work_orders.md`. TODO updated to match. |
| 2026-08-06 | **The clock stopped moving here and started moving there.** R1 fired 06:06 and shipped at 06:16 — fourteen minutes before the open — so the morning chat moves to **4:30 AM PT** (`30 11 * * 1-5`), read 5–6, acted before the bell; the session takes 10–16 minutes, so the hour was the whole problem. Underneath it, a fossil: §4.2 said "ET and PT shift together," which **B.C. ended on 2026-03-08** by going permanently UTC−7. The premise is now exactly inverted — a UTC cron holds its PT hour forever (no clock guard, no November maintenance), while the **market** moves under Zak's clock: the open is 6:30 AM PT Mar–Nov and **7:30 AM PT** Nov–Mar, and sessions must read the bell off the live ET offset instead of from memory. Accepted cost, named rather than designed around: at 4:30 the session sits 2–3 hours ahead of the bell in thin pre-market, so **gap reads are provisional** and the brief says so (§4.4, §5.1 step 3, R1 runbook). Recorded as a known limitation: Supabase's Postgres tzdata still models the old B.C. rule, so the database would render winter PT an hour early — harmless while every stamp is UTC and nothing converts (checked), fatal the day something does. |
...
| Cron | A time-based scheduler; GitHub's speaks only UTC and ignores daylight saving — harmless for Zak's clock since B.C. froze at UTC−7, but the ET-side appointments still move twice a year (§4.2) |
