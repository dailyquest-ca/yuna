...
alter table if exists research_monthly rename to research_monthly_raw_contaminated;

comment on table research_monthly_raw_contaminated is
  'DO NOT USE for analysis. Built from prices.close (the raw, split-unadjusted print), so returns '
  'across a split are meaningless — the worst is +6,474,999%. Retained as the evidence behind the '
  'census figures in docs/backtest-findings-2026-08-10.md. Use research_monthly_adj, or the '
  'research_monthly view, which points at it.';
...
comment on view research_monthly is
  'The adjusted monthly research series (research_monthly_adj). This name used to be a base table '
  'built on raw closes; it is a view now so that the unqualified name resolves to correct data '
  'regardless of what a reader knows about migration 042.';
